  ...
  while (true) {
    try {
      ...
      if (
        pre4s &&
        !mock &&
        (await tryGet4SApi(ns, playerStats, bitnodeMults, corpus * (options['buy-4s-budget'] - fracH) - reserve))
      )
...
const forecast = (history) =>
  history.reduce((ups, price, idx) => (idx == 0 ? 0 : history[idx - 1] > price ? ups + 1 : ups), 0) /
  (history.length - 1);
  ...
  for (const stk of allStocks) {
    ...
    if (!has4s)
      stk.vol = stk.priceHistory.reduce(
        (max, price, idx) => Math.max(max, idx == 0 ? 0 : Math.abs(stk.priceHistory[idx - 1] - price) / price),
        0,
      );
...
async function doBuy(ns, stk, sharesToBuy) {
  ...
  try {
    price = mock
      ? expectedPrice
      : Number(await transactStock(ns, stk.sym, sharesToBuy, long ? 'buyStock' : 'buyShort'));
  } catch (err) {
    if (long) throw err;
    disableShorts = true;
...
async function tryGet4SApi(ns, playerStats, bitnodeMults, budget) {
  ...
  if (totalCost > budget) /* Need to reserve some money to invest */ return false;
    ...
    if (await tryBuy(ns, 'purchase4SMarketData'))
      log(
        ...
        `SUCCESS: Purchased 4SMarketData for ${formatMoney(cost4sData)} ` +
          `(At ${formatDuration(getTimeInBitnode())} into BitNode)`,
        true,
        'success',
      );
